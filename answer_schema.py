...
# ── 시맨틱 슬롯 정의 ──────────────────────────────────────────────
# slot_id : (설명, [sub_category 에 포함되면 매칭되는 키워드 패턴])
SEMANTIC_SLOTS: Dict[str, Tuple[str, List[str]]] = {
    "strong_subject":   ("강점/무기/효자/타고난 과목", ["무기 과목", "효자 과목", "타고난 과목", "강점 과목"]),
    "weak_subject":     ("약점/방어/새는/취약/막막 과목", ["방어 과목", "새는 과목", "취약 과목", "막막한 과목", "약점 과목"]),
    "efficient_subject":("가성비 과목", ["가성비 과목"]),
    "subject_pref_order":("과목 선호 순서", ["과목 선호 순서", "과목 호감도", "과목별 선호"]),
    "sci_track_interest":("이과 진로 관심", ["이과 진로", "이과 분야", "이과 직업", "이과 계열"]),
    "hum_track_interest":("문과 진로 관심", ["문과 진로", "문과 분야", "문과 직업", "문과 계열"]),
    "science_subject_pref":("과학탐구 선호", ["과학탐구 선호", "과학 선호 파트", "실험vs이론", "실험·이론"]),
    "social_subject_pref":("사회탐구 선호", ["사회탐구 선호", "사회 선호 파트"]),
    "track_orientation":("문이과 성향", ["문이과 성향", "문과·이과 성향", "문이과 통합", "성적 기준 계열", "강점 과목 계열"]),
    "desired_field":    ("희망 계열", ["희망 계열", "진로 계열"]),
    "career_decided":   ("진로 결정 여부", ["진로 결정 여부", "진로 변경"]),
    "career_motive":    ("진로 선택 이유/기준", ["진로 선택 이유", "진로 선택 기준", "진로 영향"]),
    "target_univ_tier": ("목표 대학 수준", ["목표 대학"]),
    "grade_goal":       ("내신 목표 등급", ["내신 목표 등급", "주요 3과목 등급", "탐구 등급"]),
    "major_job_align":  ("전공-직업 일치 인식", ["전공-직업 일치", "잘하는 과목·전공 일치"]),
    "admission_pref":   ("수시/정시 선호", ["수시 비선호", "정시 비선호", "수시", "정시"]),
}
...
def _match_slot(sub_category: str) -> Optional[str]:
    sc = sub_category or ""
    for slot_id, (_, patterns) in SEMANTIC_SLOTS.items():
        if any(p in sc for p in patterns):
            return slot_id
    return None
...
def normalize_responses(raw_json: str) -> Dict:
    ...
    for r in responses:
        slot_id = _match_slot(r.get("sub_category", ""))
        if not slot_id:
            continue
