...
def extract_structured_text(html_text: str) -> str:
    ...
    blocks: List[str] = []
    for node in soup.find_all(["h1", "h2", "h3", "h4", "th", "td", "li", "p", "div", "span"]):
        txt = strip_text(node.get_text(" ", strip=True))
        if txt and len(txt) >= 2 and txt not in blocks:
            blocks.append(txt)
    return "\n".join(blocks)
...
def recommend_universities(db: Dict[str, Any], signals: Dict[str, Any], target_departments: List[str], top_n: int = 6) -> List[Dict[str, Any]]:
    return recommend_programs(db, signals, target_departments, top_n=top_n)
...
def fallback_summary(signals: Dict[str, Any], target_departments: List[str], recommendations: List[Dict[str, Any]]) -> str:
    ...
    university_text = ", ".join(f"{item['university']} {item.get('department','')}".strip() for item in recommendations[:6])
...
def summarize_with_gemini(signals: Dict[str, Any], target_departments: List[str], recommendations: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"추천 전형: {summarize_program_recommendations(recommendations)}\n"
...
def render_university_card(rec: Dict[str, Any], rank: int) -> None:
    ...
    dept_pct = max(0, min(100, int(rec.get("department_match", 0))))
    subject_pct = max(0, min(100, int(rec.get("subject_score", 0))))
    grade_pct = max(0, min(100, int(rec.get("grade_band_score", 0))))
    weights = "".join([f'<span class="tag-chip">{k} {v}</span>' for k, v in (rec.get("subject_weights", {}) or {}).items()])
    aliases = "".join([f'<span class="tag-chip">{x}</span>' for x in rec.get("aliases", [])[:5]])
        ...
        <div class='recommend-card'>
          <div class='recommend-head'>
            <div>
              ...
              <div class='recommend-title'>{rec['university']} · {rec['department']}</div>
              <div class='subtle'>{rec['admission_type']} · {rec.get('track_name', '')}</div>
              <div class='subtle'>{rec['region']} · {rec['campus'] if rec['campus'] else '캠퍼스 미표기'}</div>
            </div>
          ...
          <div class='score-bar-wrap'><div class='score-bar-label'>학과 일치도</div><div class='score-bar'><div class='score-fill' style='width:{dept_pct}%'></div></div></div>
          <div class='score-bar-wrap'><div class='score-bar-label'>과목 반영 적합도</div><div class='score-bar'><div class='score-fill' style='width:{subject_pct}%'></div></div></div>
          <div class='score-bar-wrap'><div class='score-bar-label'>권장 등급대 적합도</div><div class='score-bar'><div class='score-fill' style='width:{grade_pct}%'></div></div></div>
          <div class='subtle' style='margin-top:0.25rem;'>추천 신뢰도 {rec.get('recommendation_confidence', '-')} · 선택도 밴드 {rec.get('selectivity_band', '-')}</div>
          <div class='section-title'>반영 비율</div>
          <div>{weights if weights else '<span class="subtle">없음</span>'}</div>
          <div class='section-title' style='margin-top:0.8rem;'>학과 별칭</div>
          <div>{aliases if aliases else '<span class="subtle">없음</span>'}</div>
          <div class='subtle' style='margin-top:0.9rem;'>권장 등급대 {rec.get('grade_band')} · 수능최저 {rec.get('csat_minimum')}</div>
          <div class='subtle' style='margin-top:0.3rem;'>{bonus_text} · evidence {rec.get('evidence_level')}</div>
...
def main() -> None:
    ...
    with top1:
        render_metric_card("연결 DB", f"{len(db.get('universities', []))}개 대학 / {len(get_all_departments(db))}개 학과", "학과·전형 단위 확장 DB가 로드되었습니다.")
    with top2:
        render_metric_card("입력 형식", "학생 HTML", "AI 추출 + 규칙 기반 보정 파서가 함께 동작합니다.")
    with top3:
        render_metric_card("분석 방식", "전형 단위 추천", "학과·전형·반영비율·등급대 추정치를 결합해 적합도를 계산합니다.")
    ...
    with m2:
        render_metric_card("추천 전형 수", str(len(recommendations)), "근거가 부족하면 추천 수가 줄어들 수 있으며, 최대 6개 전형까지 노출합니다.")
    with m3:
        render_metric_card("추정 전체 등급", str(signals.get("overall_grade") or "-"), "HTML 서술에서 탐지한 대표 등급 값이며 전형별 권장 등급대와 비교됩니다.")
    with m4:
        render_metric_card("추출 신뢰도", str(signals.get("extraction_confidence") or "-"), "명시적 등급·학과·대학·활동 정보가 많을수록 추천 신뢰도가 올라갑니다.")
    ...
    with right:
        st.markdown("<div class='section-title'>추천 학과·전형</div>", unsafe_allow_html=True)
        ...
        st.json(
            {
                ...
                "target_departments": target_departments,
                "recommended_programs": summarize_program_recommendations(recommendations),
